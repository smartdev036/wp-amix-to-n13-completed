...
function am_add_javascript( ) {
    ...
    if( !is_admin() ) {
        ...
        if (is_page_template('page-templates/contact.php')) {
            $js = 'document.addEventListener( "wpcf7mailsent", function( event ) { location = "' . esc_url(home_url('/contact-thank-you/')). '"; }, false );';
            wp_add_inline_script( 'script', $js );
        }
    }
}
...
function am_add_css( ) {
    ...
    // internal CSS
    $am_files = array('assets/css/slick.css', 'assets/css/jquery.fancybox.min.css', '/assets/css/style.css', 'style.css'); // example: array('style1', 'style2');
    foreach($am_files as $am_file){
